...
for iter = 1 : 21
	load(sprintf('../process_result/eos_image/%d/error_final_vector', iter));
...
xlabel('\fontsize{20} log_2(\sigma_{image})');
ylabel('\fontsize{20} Average rate of mistakes');
set(gcf, 'unit', 'centimeters', 'position', [0, 0, 14.6, 11.1]);
grid;

saveas(gcf, sprintf('../process_result/eos_image/eos_image.fig'));
saveas(gcf, sprintf('../process_result/eos_image/eos_image.eps'));
saveas(gcf, sprintf('../process_result/eos_image/eos_image.pdf'));
close(figure(gcf));
