...
for i = 1 : 45
  ...
  box on;
	...
	plot(id_list, mean_mistake_list_matrix_PA_SL(i,:), 'k^-');
	%plot(id_list, mean_mistake_list_matrix_HTLIC_online(i,:), 'rs-');
	plot(id_list, mean_mistake_list_matrix_HetOTL(i,:), 'rs-');
  ...
  legend0 = legend('PA', 'PANV', 'PASL', 'HetOTL', 'OTLHS');
  ...
  xlabel('\fontsize{20} Number of samples');
	...
	saveas(gcf, sprintf('./figure/%d.eps', i));
	saveas(gcf, sprintf('./figure/%d.pdf', i));
	close(figure(gcf));
end
%========================================================
