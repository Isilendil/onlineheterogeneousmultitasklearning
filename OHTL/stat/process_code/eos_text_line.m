...
for iter = 1 : 21
	load(sprintf('../process_result/eos_text/%d/error_final_vector', iter));
...
xlabel('\fontsize{20} log_2(\sigma_{text})');
ylabel('\fontsize{20} Average rate of mistakes');
set(gcf, 'unit', 'centimeters', 'position', [0, 0, 14.6, 11.1]);
grid;

saveas(gcf, sprintf('../process_result/eos_text/eos_text.fig'));
saveas(gcf, sprintf('../process_result/eos_text/eos_text.eps'));
saveas(gcf, sprintf('../process_result/eos_text/eos_text.pdf'));
close(figure(gcf));
