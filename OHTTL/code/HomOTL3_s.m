function [classifier, err_count, run_time, mistakes, mistakes_idx, SVs, TMs] = HomOTL3_s(Y, K1, K2, options, id_list,classifier)
% HomOTL2: HomOTL-II
%--------------------------------------------------------------------------
% Input:
...
for t = 1:length(ID),
    ...
    %z_1 = (Y(id)*f1_t<=0);
    %z_2 = (Y(id)*f2_t<=0);
    z_1 = 1/(1+exp(Y(id)*f1_t));
    z_2 = 1/(1+exp(Y(id)*f2_t));
    w_1t=w_1t*beta^z_1;
    w_2t=w_2t*beta^z_2;
    sum_w=w_1t+w_2t;
    w_1t=w_1t/sum_w;
