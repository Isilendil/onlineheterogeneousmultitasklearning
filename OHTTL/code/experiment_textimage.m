...
load(sprintf('sourcecodata/textimage/%d', data_file));

load('tool/ID_textimage');


% data
X_source = co_image_fea;
...
%temp = 'birds-food-1';
%load(sprintf('../../../../OMT/TextImage/data/feature1/%s', temp));
%X_htlic = image_fea_1;
...
%K_htlic = exp(-(repmat(P_htlic',n_target,1) + repmat(P_htlic,1,n_target)- 2*X_htlic*X_htlic')/(2*options.sigma^2));
...
stat_file = sprintf('../stat/textimage/%d', data_file);
save(stat_file, 'nSV_pa', 'err_pa', 'time_pa', 'mistakes_list_pa', 'SVs_pa', 'TMs_pa', 'nSV_htlic', 'err_htlic', 'time_htlic', 'mistakes_list_htlic', 'SVs_htlic', 'TMs_htlic', 'nSV_het', 'err_het', 'time_het', 'mistakes_list_het', 'SVs_het', 'TMs_het', 'nSV_pca', 'err_pca', 'time_pca', 'mistakes_list_pca', 'SVs_pca', 'TMs_pca', 'nSV_OTL_svm', 'err_OTL_svm', 'time_OTL_svm', 'mistakes_list_OTL_svm', 'SVs_OTL_svm', 'TMs_OTL_svm', 'nSV_OTL2_svm', 'err_OTL2_svm', 'time_OTL2_svm', 'mistakes_list_OTL2_svm', 'SVs_OTL2_svm', 'TMs_OTL2_svm', 'nSV_OTL_tsvm', 'err_OTL_tsvm', 'time_OTL_tsvm', 'mistakes_list_OTL_tsvm', 'SVs_OTL_tsvm', 'TMs_OTL_tsvm', 'nSV_OTL2_tsvm', 'err_OTL2_tsvm', 'time_OTL2_tsvm', 'mistakes_list_OTL2_tsvm', 'SVs_OTL2_tsvm', 'TMs_OTL2_tsvm', 'nSV_OTL_knn', 'err_OTL_knn', 'time_OTL_knn', 'mistakes_list_OTL_knn', 'SVs_OTL_knn', 'TMs_OTL_knn', 'nSV_OTL2_knn', 'err_OTL2_knn', 'time_OTL2_knn', 'mistakes_list_OTL2_knn', 'SVs_OTL2_knn', 'TMs_OTL2_knn', 'nSV_OTL_pa', 'err_OTL_pa', 'time_OTL_pa', 'mistakes_list_OTL_pa', 'SVs_OTL_pa', 'TMs_OTL_pa');


%{
'nSV_pa', 'err_pa', 'time_pa', 'mistakes_list_pa', 'SVs_pa', 'TMs_pa', 
'nSV_htlic', 'err_htlic', 'time_htlic', 'mistakes_list_htlic', 'SVs_htlic', 'TMs_htlic', 
'nSV_het', 'err_het', 'time_het', 'mistakes_list_het', 'SVs_het', 'TMs_het', 
'nSV_pca', 'err_pca', 'time_pca', 'mistakes_list_pca', 'SVs_pca', 'TMs_pca', 
'nSV_OTL_svm', 'err_OTL_svm', 'time_OTL_svm', 'mistakes_list_OTL_svm', 'SVs_OTL_svm', 'TMs_OTL_svm', 
'nSV_OTL2_svm', 'err_OTL2_svm', 'time_OTL2_svm', 'mistakes_list_OTL2_svm', 'SVs_OTL2_svm', 'TMs_OTL2_svm', 
'nSV_OTL_tsvm', 'err_OTL_tsvm', 'time_OTL_tsvm', 'mistakes_list_OTL_tsvm', 'SVs_OTL_tsvm', 'TMs_OTL_tsvm', 
'nSV_OTL2_tsvm', 'err_OTL2_tsvm', 'time_OTL2_tsvm', 'mistakes_list_OTL2_tsvm', 'SVs_OTL2_tsvm', 'TMs_OTL2_tsvm', 
...
fprintf(1,'HTLIC             %.4f \t %.4f \t %.4f \t %.4f \t %.4f \t %.4f\n', mean(err_htlic)/m*100,  std(err_htlic)/m*100, mean(nSV_htlic), std(nSV_htlic), mean(time_htlic), std(time_htlic));
fprintf(1,'HET             %.4f \t %.4f \t %.4f \t %.4f \t %.4f \t %.4f\n', mean(err_het)/m*100,  std(err_het)/m*100, mean(nSV_het), std(nSV_het), mean(time_het), std(time_het));
fprintf(1,'PCA             %.4f \t %.4f \t %.4f \t %.4f \t %.4f \t %.4f\n', mean(err_pca)/m*100,  std(err_pca)/m*100, mean(nSV_pca), std(nSV_pca), mean(time_pca), std(time_pca));
...
fprintf(1,'HomOTL_SVM       %.4f \t %.4f \t %.4f \t %.4f \t %.4f \t %.4f\n', mean(err_OTL_svm)/m*100,  std(err_OTL_svm)/m*100, mean(nSV_OTL_svm), std(nSV_OTL_svm), mean(time_OTL_svm), std(time_OTL_svm));
fprintf(1,'HomOTL2_SVM      %.4f \t %.4f \t %.4f \t %.4f \t %.4f \t %.4f\n', mean(err_OTL2_svm)/m*100, std(err_OTL2_svm)/m*100, mean(nSV_OTL2_svm), std(nSV_OTL2_svm), mean(time_OTL2_svm), std(time_OTL2_svm));
fprintf(1,'HomOTL_TSVM       %.4f \t %.4f \t %.4f \t %.4f \t %.4f \t %.4f\n', mean(err_OTL_tsvm)/m*100,  std(err_OTL_tsvm)/m*100, mean(nSV_OTL_tsvm), std(nSV_OTL_tsvm), mean(time_OTL_tsvm), std(time_OTL_tsvm));
fprintf(1,'HomOTL2_TSVM      %.4f \t %.4f \t %.4f \t %.4f \t %.4f \t %.4f\n', mean(err_OTL2_tsvm)/m*100, std(err_OTL2_tsvm)/m*100, mean(nSV_OTL2_tsvm), std(nSV_OTL2_tsvm), mean(time_OTL2_tsvm), std(time_OTL2_tsvm));
