...
image = mean(mean_error_OGD_personal_image);
...
text = mean(mean_error_OGD_personal_text);
...
set(gcf, 'unit', 'centimeters', 'Position', [0, 0, 14.6, 11.1]);
legend0 = legend('PA-personal', 'PA-shared', 'OGD-personal', 'PA-extra-sim', 'PA-extra');
%legend0 = legend('PA-personal', 'PA-shared', 'OGD_personal', 'dso1', 'PA-extra-sim', 'PA-extra');
set(legend0, 'Fontsize', 15);
ymin = min(text);
ymax = max(text);
axis([0, 6, floor(ymin/5)*5, ceil(ymax/5)*5]);
